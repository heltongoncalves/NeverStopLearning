Products.English.Exercises.Semester1.Exercise003 = {};
Products.English.Exercises.Semester1.Exercise003.WordsToNumbers = new Process("WordsToNumbers", function onDefineProcess(){
	...
	function process_NumberGenerator( fld, oldValue, newValue ){
		 ...
		 for (var i=1; i<=maxNumbers; i++){
			...
			f.numberToWords = Products.English.EnglishAuxiliaryFunctions.numberToEnglish( number+'' ).trim();
			f.wordsToNumber = number+'';
			f.setLabel(f.numberToWords);
			f.warning();
		}			 	 
	...
	function numbersAndWordsComparing( fld, valueToCompare ){
		if ( valueToCompare ){
			if ( (valueToCompare+"").trim().toLowerCase().replace('  ', ' ') == fld.wordsToNumber.trim().toLowerCase().replace('  ', ' ') ){
				fld.success();
				return true;
			} else {
				fld.danger();
